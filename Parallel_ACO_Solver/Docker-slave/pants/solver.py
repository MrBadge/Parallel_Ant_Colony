...
class Solver:
    ...
    :param float rho: percent evaporation of pheromone (0..1, default=0.7)
    ...
    def __init__(self, **kwargs):
        ...
        self.rho = kwargs.get('rho', 0.7)
        self.q = kwargs.get('Q', 1)
        self.t0 = kwargs.get('t0', .01)
        self.iterations = kwargs.get('iterations', 100)
        self.ant_count = kwargs.get('ant_count', 10)
        self.ant_capacity = kwargs.get('ant_capacity', 100)
        self.elite = kwargs.get('elite', .3)

    def create_colony(self, world, vrp=True):
        ...
        *world*.
        ...
        """
        if vrp:
            return self.VRP_ants(world, self.ant_count)
        if self.ant_count < 1:
    ...
    def reset_colony(self, colony):
        ...
        """
        for ant in colony:
            ant.initialize(ant.world, start=ant.start)
    ...
    def solve(self, world):
        ...
        for i in range(self.iterations):
            ...
            if global_best is None or local_best < global_best:
                ...
                logging.debug('Found better solution on %s iteration!' % (no_changes_count + 1))
                no_changes_count = 0
            else:
                no_changes_count += 1
            self.trace_elite(global_best)
            if no_changes_count > 20:
    ...
    def VRP_ants(self, world, count):
        starts = world.nodes
        return [
            Ant(self.alpha, self.beta, capacity=self.ant_capacity).initialize(
    ...
    def round_robin_ants(self, world, count):
        ...
        n = len(starts)
        return [
            Ant(self.alpha, self.beta, self.ant_capacity).initialize(
        ...
        while ants_done < len(ants):
            ...
            for ant in ants:
                if ant.can_move():
                    ...
                    if ant.node == ant.start:
                        ants_done += 1
                        continue
                else:
                    if ant.node != 0:
                        edge = ant.world.data(ant.node, 0)
                        ant.distance += edge.length
                    ants_done += 1
    ...
    def global_update(self, ants):
        ...
        for a in ants:
            p = self.q / (a.distance or float('inf'))
            for edge in a.path:
                edge.pheromone = max(
                    self.t0,
    ...
    def trace_elite(self, ant):
        ...
        """
        if self.elite:
            p = self.elite * self.q / (ant.distance or float('inf'))
